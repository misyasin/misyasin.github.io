...
# Function to calculate rate history factor based on user ID
def get_rate_history_factor(user_id):
    return 0.01 if has_rate_history(user_id) else 0.00
...
# ======================== end pricing module related code ========================
    ...
    while History.query.filter_by(id=user_id).first():
        user_id_id = random.randint(1, 10000)
...
def get_quote_history(user_id):
    ...
    try:
        ...
        return jsonify({'error': 'Invalid User ID. Please provide a valid integer ID.'}), 400

    history = History.query.get(user_id_int)
    if history:
        return jsonify({
            'history': [
                {
